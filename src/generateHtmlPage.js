...
function generateManagerCards(managerTitle) {
  return managerTitle
    .map((manager) => {
...
function generateInternCards(internTitle) {
  return internTitle
    .map((intern) => {
      return `<div class="col">
    <div class="card shadow-lg" style="width: 18rem;">
...
function generateEngineerCards(engineerTitle) {
...
// Generates cards
function generateCardObjects(teams) {
  let allCards = [];
  const managerTeam = teams.filter((team) => {
    return team.getRole() == "Manager";
  });

  const engineerTeam = teams.filter((team) => {
    return team.getRole() == "Engineer";
  });
  const internTeam = teams.filter((team) => {
    return team.getRole() == "Intern";
  });

  if (managerTeam && managerTeam.length > 0) {
    console.log(managerTeam);
    allCards.push(generateManagerCards(managerTeam));
  }

  if (engineerTeam && engineerTeam.length > 0) {
    console.log(engineerTeam);
    allCards.push(generateEngineerCards(engineerTeam));
  }
  if (internTeam && internTeam.length > 0) {
    console.log(internTeam);
    allCards.push(generateInternCards(internTeam));
  }
...
<body>
    ...
    <main class="container my-5">
        ...
        <div class="row">
            ...
            ${generateCardObjects(cardsArray)}
